...
export default function ResearchExperience() {
  const items = EXPERIENCES.filter(e => e.type === "academic");

  return (
    <section id="research-experience" className="section-gap bg-[var(--bg-alt)]">
            ...
            >
              ...
              <div className="flex-1 pt-0.5">
                <div className="flex flex-wrap items-baseline gap-2 mb-0.5">
                  <span className="font-serif text-[0.95rem] font-medium text-[var(--fg)]">{exp.role}</span>
                  {exp.current && <span className="tag tag-green">current</span>}
                </div>
                <p className="text-sm text-[var(--accent)] mb-1">{exp.company}</p>
                {"supervisor" in exp && exp.supervisor && (
                  <p className="text-xs text-[var(--fg-faint)] mb-1.5">
                    Supervisor:{" "}
                    {"supervisorUrl" in exp && exp.supervisorUrl ? (
                      <a href={exp.supervisorUrl as string} target="_blank" rel="noopener noreferrer"
                        className="text-[var(--accent)] hover:underline">
                        {exp.supervisor as string}
                      </a>
                    ) : (
                      <span className="text-[var(--fg-muted)]">{exp.supervisor as string}</span>
                    )}
                  </p>
                )}
